...
def create_username(first_name, last_name, existing_usernames):
...
def main():
    ...
    for full_name in user_names:
        ...
        if len(parts) < 2:
            print(f"Uyarı: '{full_name}' geçersiz format, atlanıyor...")
            continue
        ...
        username = create_username(first_name, last_name, existing_usernames)
        ...
        if existing_user:
            print(f"Atlandı: {full_name} (zaten mevcut)")
            skipped_count += 1
            continue
        ...
        try:
            user = CustomUser.objects.create_user(
                ...
                first_name=first_name,
                last_name=last_name,
                password='123456'  # Varsayılan şifre, daha sonra değiştirilebilir
            )
            print(f"Oluşturuldu: {full_name} (kullanıcı adı: {username})")
            created_count += 1
        except Exception as e:
            print(f"Hata: {full_name} oluşturulamadı - {e}")
